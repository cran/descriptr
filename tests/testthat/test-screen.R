...
test_that("output from screener matches the expected result", {
    ...
    expect_equivalent(k$Types, c("numeric", "factor", "numeric", "numeric",
                                 "numeric", "numeric", "numeric",
                                 "factor", "factor", "factor", "factor" ))
    expect_equal(unname(k$Count), rep(32, 11))
    expect_equivalent(k$nlevels, list(mpg = 0, cyl = 3, disp = 0, hp = 0,
                                      drat = 0, wt = 0, qsec = 0, vs = 2,
                                      am = 2, gear = 3, carb = 6))
    expect_equivalent(k$levels, list(mpg = NA, cyl = c("4", "6", "8"),
                                     disp = NA, hp = NA, drat = NA, wt = NA,
                                     qsec = NA, vs = c("0", "1"),
                                     am = c("0", "1"), gear = c("3", "4", "5"),
                                     carb = c("1", "2", "3", "4", "6", "8")))
    expect_equivalent(k$Missing, c(mpg = 0, cyl = 12, disp = 0, hp = 12,
                                      drat = 0, wt = 0, qsec = 12, vs =12,
                                      am = 12, gear = 12, carb = 0))
    expect_equivalent(k$MissingPer, c(mpg = 0, cyl = 37.5, disp = 0, hp = 37.5,
